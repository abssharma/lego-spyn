...
if(toc(timerVal) > 3.67) %State transition after 4 seconds
brick.StopMotor('A');
...
pause(.5);
brick.MoveMotor('A', 60);
if(toc(timerVal)> 3.70) %State transition after 4 second
timerVal = tic;
state = 5;
end
...
if(toc(timerVal) > 3.69) %State transition after 3.75 seconds
brick.StopMotor('A');
timerVal = tic; %Reset timer
state = 0;
end
